...
This document is the binding API standard for Bliq. Cursor must follow it for all new or modified API routes.

Bliq is a web + iOS + Android platform. APIs must be stable, predictable, mobile-friendly, and tenant-safe.

---

## 1. Route Style

Bliq uses tenant-nested API routes.
...
All Bliq APIs must use a predictable JSON response shape.
