# Bliq phase status (elite pack — reconciled with repo)

## Purpose

This file is part of the **elite docs** pack. The original pack used a **different phase numbering** than this repository.

**Canonical implementation phases:** [../BLIQ_BUILD_PLAN.md](../BLIQ_BUILD_PLAN.md) **Part D** (Phase **0–7**).

**Delivery slices after the core API:** [../TRANCHES.md](../TRANCHES.md) (T0–T6).

Cursor should use **BLIQ_BUILD_PLAN Part D** for “what phase are we in?” and this file for **crosswalk + release-oriented gaps**.

---

## Global build state (this repo)

Bliq is **API-first** with a thin web shell. Foundation through **Phase 7 (Stripe PaymentIntent + webhook + `Payment` on success)** is implemented per Part D; see [../BLIQ_STATUS_SUMMARY.md](../BLIQ_STATUS_SUMMARY.md).

---

## Canonical status — BLIQ_BUILD_PLAN Part D
...
Do not derail into storefronts, full messaging, or heavy AI until **T2** is credible — see [BLIQ_SOURCE_OF_TRUTH.md](./BLIQ_SOURCE_OF_TRUTH.md) §8 and [../TRANCHES.md](../TRANCHES.md).
...
```text
Read docs/BLIQ_BUILD_PLAN.md (Part A + Part D), docs/elite/README.md, docs/elite/BLIQ_PHASE_STATUS.md (this file),
docs/elite/BLIQ_API_STANDARD.md for API shape, and prisma/schema.prisma. Then implement the requested work.
```
