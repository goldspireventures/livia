...
Livia is built for EU/Ireland appointment-based service businesses. Tenant data — bookings, customers, conversations, AI training material, audit logs — lives in the **EU/IRE region** of our hosting provider and stays there. Cross-region replication for disaster recovery, where it exists, is to other EU regions only.
...
|---|---|---|---|---|
| **Replit Deployments + Replit Object Storage** | Compute, hosting, file storage (logos, customer photos). | All tenant data on disk + at runtime. | EU (configured per Lane 4 L11). | Processor under our DPA. |
| **Postgres (Replit-managed)** | Tenant database. | All tenant data. | EU (configured at provisioning). | Processor under our DPA. |
...
| **Anthropic (via Replit AI Integrations proxy)** | LLM inference for Liv replies, AI training generation. | Prompts (which include conversation context), system prompts (which include shop tone/knowledge), tool-call payloads. | Routed via the proxy; Anthropic processes in their default region. **No training on customer data** — Anthropic API tier is zero-retention for training. | Processor under Anthropic Commercial Terms + Replit's pass-through DPA. |
...
- Object Storage buckets — created in EU region only.
- Customer photos, before/after images, signatures — EU-only Object Storage prefix.
...
- **Export** (`POST /api/me/export`) — JSON + CSV bundle of all tenant data, delivered by email, EU-only S3 presigned URL, expires in 24h. (Compliance C3.)
- **Erasure** — soft-delete + 30-day purge job. Audit log entry retained for 1 year as proof. (Compliance C4.)

## Annual review

This document is reviewed annually by the founder + counsel. Last review: 2026-05-06. Next review: 2027-05.

## EU/IRE residency

All tenant data, every audit row, every billing record mirror, every Object Storage prefix lives in the EU/Ireland region. This is the foundational guarantee Livia is built on.
