# Livia — where we are now (snapshot)

_Generated for product/engineering alignment; update as releases ship._

## Business / product

**What Livia is:** A **mobile-first, multi-tenant operating system** for appointment-based service businesses — generic **Business → Staff → Service → Customer → Booking** model (not a single-vertical app).
...
| **Docs** | `LIVIA_BUILD_PLAN` (source of truth), `ROADMAP`, `MASTER_SPINE` + appendices, **`docs/elite/`** (imported standards pack), `REPO_DELTA`, `FOLDER_STRUCTURE`. |
