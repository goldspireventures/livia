# Bliq — Build Plan & Source of Truth

**Foundational document.** Agents must read this file first to understand Bliq as a product and platform, non‑negotiable engineering rules, and product philosophy. Then read `prisma/schema.prisma`, relevant `src/services/*`, and `docs/CURSOR_RULES.md` before changing code.

**Roadmap index:** [docs/ROADMAP.md](./ROADMAP.md) explains how Part D **phases** (0–7+) relate to a longer master product spec (section numbers are not the same as phases). **Master spec spine + appendices:** [docs/MASTER_SPINE.md](./MASTER_SPINE.md). **BLIQ ↔ master index:** [docs/MASTER_BLIQ_INDEX.md](./MASTER_BLIQ_INDEX.md). **Tranches (T0–T6):** [docs/TRANCHES.md](./TRANCHES.md). **Folder layout:** [docs/FOLDER_STRUCTURE.md](./FOLDER_STRUCTURE.md). **Spec vs repo:** [docs/REPO_DELTA.md](./REPO_DELTA.md). **Product/infra snapshot:** [docs/BLIQ_STATUS_SUMMARY.md](./BLIQ_STATUS_SUMMARY.md). **Elite standards pack (imported):** [docs/elite/README.md](./elite/README.md).
...
Bliq is a **mobile-first, multi-tenant, AI-ready operating system for service businesses**.
...
> Bliq replaces fragmented tools (booking apps, spreadsheets, messaging, payments, admin) with a single unified system.

---

### 2. Core Product Vision

Bliq helps businesses manage:
...
Bliq must support barbers, salons, nail techs, lash techs, tutors, personal trainers, tattoo artists, consultants, mechanics, mobile/home service providers, and similar appointment-based businesses.
...
Bliq is not only a backend. Businesses should have **public-facing** pages, brand presence, and **booking entry points**.
...
Bliq should feel **fast**, **clean**, **modern**, **premium**, and **simple**.
...
> Build Bliq as if it will power **thousands** of businesses, not just one.
...
```text
Implement Phase 1 of Bliq: Business + Membership Foundation.

Context:
Bliq is a multi-tenant service-business operating platform. Business is the tenant root. Business logic belongs in src/services. API routes must be thin.
...
```text
Implement the next Bliq phase per docs/BLIQ_BUILD_PLAN.md (read Part A first).
