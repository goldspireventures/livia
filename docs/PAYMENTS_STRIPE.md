...
2. **Stripe** sends lifecycle events to **`/api/webhooks/stripe`**. Livia updates `PaymentIntentRecord` and inserts **`Payment`** on success.
...
- Unknown PaymentIntents (no matching Livia row) log `STRIPE_WEBHOOK_UNKNOWN_INTENT`; the HTTP handler returns `{ received: true }` when the Stripe signature is valid so Stripe does not infinite-retry a payload you intentionally ignore.
